...
import { approveCreative, rejectCreative, setCreativeStatus } from "@/lib/admin-actions";
...
export default async function ApplicationsPage({
  ...
  const creatives = await db.creative.findMany({
    where: status === "ALL" ? {} : { status: status as "PENDING" | "APPROVED" | "REJECTED" | "INACTIVE" },
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        ...
        <div className="flex gap-2 text-sm">
          {["PENDING", "APPROVED", "REJECTED", "INACTIVE", "ALL"].map((s) => (
            <a
              key={s}
